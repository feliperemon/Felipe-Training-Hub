...
const PAGE_IDS = {
  ...
  schedule: '320e6abcb90281bf9b43c9514a529c97',
};
...
async function queryDB(dbId, sorts, pageSize = 15) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    configTable.rows.forEach(row => {
      ...
      const notes = row['Notes'] || row[configTable.header[2]] || '';
      if (key) {
        config[key] = val;
        if (notes) config[`${key}__notes`] = notes;
      }
    });
    ...
    const weeklyRaw = await queryDBFiltered(
      DB_IDS.workoutLog,
      { property: 'Date', date: { on_or_after: monday.toISOString().split('T')[0] } },
      [{ property: 'Date', direction: 'descending' }],
      30
    );
